    ...
    try:
        ...
        if r.recognize_google(audio, language='RU-ru').lower().find('дай') != -1:
            ...
            if r.recognize_google(audio, language='RU-ru').lower().find('стакан') != -1:
                ...
                d = "banan"
                break
            elif r.recognize_google(audio, language='RU-ru').lower().find('игрушка') != -1:
                d = "potted plant"
                break
            else:
                print('Объекта нет')
...
if d in l:
    ...
    if d=="cup" or d == "potted plant":
        while True: ## NOTE: Около 10 сигналов он кинет.
            ser.write(b"c") ## NOTE: Отправка плюсика на чашку и игрушку.
            print('ОТПРАВЛЕНО: [C] ' + d)
    elif d == "banan":
        while True:
            ser.write(b"b") ## NOTE: Отправка плюсика на банан.
            print('ОТПРАВЛЕНО: [B] ' + d)

else:
    while True:
        ser.write(b"h") ## NOTE: Отправка +угла.
        print('ОТПРАВЛЕНО: [H] ' + d) ## NOTE: Отправка .
        ## WARNING: Завершить килл процесса отправки на сериал и обновить цикл поиска.
        ##################
        ## WARNING: Если ардуинка не нашла - отдает на вывод сканеру снова, посредством принятия сериала.
l=[]
